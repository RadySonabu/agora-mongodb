...
async def get_data(limit: int = 10, offset:int=0, request: Request = any):
    ...
    if data:
        return PaginatedResponseModel(data=data['data'], request=request, count=data['count'], offset=offset, limit=limit,route='tags', message="data data retrieved successfully")
